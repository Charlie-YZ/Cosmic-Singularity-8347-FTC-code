...
public class Duo extends LinearOpMode {
    ...
    private boolean firstFull = true;
    ...
    public void autoAlign(){
        while(vision.getTxRaw() >= 1.0 || vision.getTxRaw() <= -1.0){
            if(vision.getTxRaw() <= 2.0 && vision.getTxRaw() <= 2.0){
                drivetrain.autoRotatePower = (-vision.getTxRaw()*0.0025);
            }
            if(vision.getTxRaw() <= 4.0 && vision.getTxRaw() <= 4.0){
                drivetrain.autoRotatePower = (-vision.getTxRaw()*0.005);
            }
            drivetrain.autoRotatePower = (-vision.getTxRaw()*0.01);
        }
    }
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            // X 切换射击模式
            if(transfer.ballcontain >= 3 && firstFull){
            ...
            if (gamepad1.y) drivetrain.resetIMU();
            if (gamepad2.dpad_up){
                vision.kp += 0.001;
                sleep(200);
